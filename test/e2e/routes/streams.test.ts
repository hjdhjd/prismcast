 ...
 *
 * The projection's default-fill path is the load-bearing detail: a freshly registered stream has never emitted a status, so getStreamStatus() returns undefined
 * and every status-derived field must fall back through its ?? default (health "healthy", clientCount 0, clients [], escalationLevel 0, logoUrl "",
