...
export function generateStatusScript(): string {
  ...
  return [
    ...
    "    var hwIcon = s.hardwareAccelerated ? '\\u26A1 ' : '';",
    "    var nativeBadge = s.streamingMode === 'native' ? ' <span class=\"native-badge\" title=\"Native HLS\">' + hwIcon + 'Native</span>' :",
    "      (s.hardwareAccelerated ? ' <span class=\"native-badge\" title=\"Hardware accelerated\">' + hwIcon + (s.captureCodec || '') + '</span>' : '');",
    ...
    "      var codecLabel = s.captureCodec ? (s.hardwareAccelerated ? '\\u26A1 ' + s.captureCodec : s.captureCodec) :",
    "        (s.streamingMode === 'native' ? 'Native HLS' : 'Unknown');",
    "      var modeLabel = s.streamingMode === 'native' ? 'Native HLS' : 'Capture';",
    "      html += '<div class=\"details-codec\"><strong>Codec:</strong> ' + codecLabel + ' (' + modeLabel + ')</div>';",
    ...
    "  var buttons = document.querySelectorAll('.btn-icon-login[data-auth-domain=\"' + domain + '\"]');",
    ...
    "    updateDomainAuth(key, data.domains[key]);",
    "  }",
    "}",
