...
export function generateChannelsSubtabScript(): string {
  ...
  return [
    ...
    "        if(!r.ok) { const d = await r.json(); throw new Error(d.error || 'Discovery failed.'); }",
    ...
    "      const data = await res.json();",
    "      if(!data.success) {",
    "        showToast(data.error || 'Failed to restore tag.', 'error');",
    "        return;",
    "      }",
    ...
    "        if(!data.success) {",
    "          restore();",
    "          showToast(data.error || 'Failed to rename tag.', 'error');",
    "          return;",
    "        }",
