...
export function generateChannelsSubtabScript(): string {
  ...
  return [
    ...
    "          const sumTag = p.summary ? ' <span class=\"profile-option-summary\">(' + esc(p.summary) + ')</span>' : '';",
    ...
    "      const domainList = s.domains.filter((d) => d.domain).map((d) => esc(d.domain)).join(', ');",
    "      html += '<tr><td>Domain(s)</td><td>' + (domainList || 'none') + '</td></tr>';",
    "      const serviceList = s.domains.filter((d) => d.service).map((d) => esc(d.service)).join(', ');",
    ...
    "      html += '<label class=\"' + itemClass + '\" data-name=\"' + esc(searchName).replaceAll('\"', '&quot;') + '\">';",

    // Checkbox with data attributes for state tracking and submission.
    "      html += '<input type=\"checkbox\" class=\"browse-channel-cb\"';",
    "      html += ' data-name=\"' + esc(ch.name).replaceAll('\"', '&quot;') + '\"';",
    "      html += ' data-selector=\"' + esc(ch.channelSelector).replaceAll('\"', '&quot;') + '\"';",
    ...
    "    const label = useExclude ? 'all channels except those tagged <strong>' + unchecked.join(', ') + '</strong>'",
