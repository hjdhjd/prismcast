...
export function generateChannelsSubtabScript(): string {
  ...
  return [
    ...
    "  window.deleteUserProfile = async function(key) {",
    "    if(!confirm('Delete profile \"' + key + '\" and all its domain mappings?')) return;",
    "    try {",
    "      var res = await fetch('/config/profiles/' + encodeURIComponent(key), { method: 'DELETE' });",
    ...
    "    } catch(err) { console.error('Profile delete error:', err); showToast('Failed to delete profile.', 'error'); }",
    "  };",

    // Edit an existing user profile by pre-populating the wizard.
    "  window.editUserProfile = async function(key) {",
    "    try {",
    ...
    "    try {",
    "      var res = await fetch('/config/profiles/import', {",
    "        method: 'POST',",
    ...
    "  window.saveProfile = async function(andTest) {",
    "    var s = profileWizard.state;",
    ...
    "  window.startProfileTest = async function(url) {",
    "    var s = profileWizard.state;",
    "    testSelectors = {};",
    ...
    "    try {",
    "      var res = await fetch('/config/profiles/test', {",
    "        method: 'POST',",
    "        headers: { 'Content-Type': 'application/json' },",
    ...
    "      }",
    "    } catch(e) { showToast('Failed to start test.', 'error'); }",
    "  };",

    // Check CSS selectors against the live page.
    "  window.checkSelectors = async function() {",
    "    if(Object.keys(testSelectors).length === 0) return;",
    "    try {",
    "      var res = await fetch('/config/profiles/test/check', {",
    "        method: 'POST',",
    "        headers: { 'Content-Type': 'application/json' },",
    ...
    "  window.endProfileTest = async function() {",
    "    try {",
    "      var res = await fetch('/config/profiles/test/done', { method: 'POST' });",
    "      var data = await res.json();",
    "      document.getElementById('test-modal').style.display = 'none';",
    "      if(data.panelHtml) { applyProfilePanel(data.panelHtml); }",
    "    } catch(err) {",
    "      console.error('End profile test error:', err);",
    "      document.getElementById('test-modal').style.display = 'none';",
    "    }",
    "  };",
    ...
    "    (async function() {",
    "      try {",
    "        var res = await fetch('/config/channels/modify', {",
    "          method: 'POST',",
    "          headers: { 'Content-Type': 'application/json' },",
    ...
    "    try { await fetch('/config/channels/setup-completed', { method: 'POST', headers: { 'Content-Type': 'application/json' } }); }",
    "    catch(e) {}",
    "    setupWizard.close();",
    "  };",
    ...
    "  window.applyTagResponse = function(data, toastMessage) {",
    "    var contentDiv = document.getElementById('tag-manager-modal-content');",
    ...
    "    var modal = document.getElementById('tag-manager-modal');",
    ...
    "    var errorDiv = document.getElementById('tag-manager-error');",
    "    var tag = input ? input.value.trim().toLowerCase() : '';",
    "    if(!tag) { return; }",
    ...
    "    fetch('/config/tags/' + encodeURIComponent(tag), {",
    "      method: 'DELETE'",
    "    }).then(function(res) { return res.json(); }).then(function(data) {",
    "      if(!data.success) {",
    "        if(window.showToast) { window.showToast(data.error || 'Failed to delete tag.', 'error'); }",
    "        return;",
    "      }",
    "      applyTagResponse(data, 'Tag deleted: ' + tag);",
    "    }).catch(function() {",
    "      if(window.showToast) { window.showToast('Failed to delete tag.', 'error'); }",
    "    });",
    "  };",

    // Restore a deleted predefined tag via the API, then update the modal and channel table incrementally.
    "  window.restoreTag = function(tag) {",
    "    fetch('/config/tags/restore', {",
    "      method: 'POST',",
    "      headers: { 'Content-Type': 'application/json' },",
    "      body: JSON.stringify({ tag: tag })",
    "    }).then(function(res) { return res.json(); }).then(function(data) {",
    "      if(!data.success) {",
    "        if(window.showToast) { window.showToast(data.error || 'Failed to restore tag.', 'error'); }",
    "        return;",
    "      }",
    "      applyTagResponse(data, 'Tag restored: ' + tag);",
    "    }).catch(function() {",
    "      if(window.showToast) { window.showToast('Failed to restore tag.', 'error'); }",
    "    });",
    "  };",
    ...
    "  window.applyTagColumnFilter = function() {",
    "    var checkboxes = document.querySelectorAll('.tag-filter-checkbox');",
    "    var checkedTags = [];",
    "    var allChecked = true;",
    "    for(var i = 0; i < checkboxes.length; i++) {",
    "      if(checkboxes[i].checked) { checkedTags.push(checkboxes[i].getAttribute('data-tag')); }",
    "      else { allChecked = false; }",
    "    }",
    "    var rows = document.querySelectorAll('tr[id^=\"display-row-\"]');",
    "    for(var j = 0; j < rows.length; j++) {",
    "      var row = rows[j];",
    "      var editRow = document.getElementById(row.id.replace('display-row-', 'edit-row-'));",
    "      if(allChecked) {",
    "        row.classList.remove('tag-filtered');",
    "        if(editRow) editRow.classList.remove('tag-filtered');",
    "        continue;",
    "      }",
    "      var tags = row.getAttribute('data-channel-tags');",
    "      var channelTags = tags ? tags.split(',') : [];",
    "      var visible = false;",
    "      for(var k = 0; k < channelTags.length; k++) {",
    "        if(checkedTags.indexOf(channelTags[k]) !== -1) { visible = true; break; }",
    "      }",
    "      if(visible) { row.classList.remove('tag-filtered'); if(editRow) editRow.classList.remove('tag-filtered'); }",
    "      else { row.classList.add('tag-filtered'); if(editRow) editRow.classList.add('tag-filtered'); }",
    "    }",
    ...
    "    var hintBtn = document.getElementById('playlist-hint-btn');",
    ...
    "    var toggle = document.getElementById('tag-filter-toggle');",
    ...
    "  window.toggleTagColumnFilter = function() {",
    "    var checkboxes = document.querySelectorAll('.tag-filter-checkbox');",
    "    var allChecked = true;",
    ...
    "  window.showPlaylistHint = function(btn) {",
    "    var checkboxes = document.querySelectorAll('.tag-filter-checkbox');",
    "    var checked = [];",
    "    var unchecked = [];",
    "    for(var i = 0; i < checkboxes.length; i++) {",
    "      var tag = checkboxes[i].getAttribute('data-tag');",
    "      if(checkboxes[i].checked) { checked.push(tag); }",
    "      else { unchecked.push(tag); }",
    "    }",
    "    var useExclude = unchecked.length < checked.length;",
    "    var tagParam = useExclude ? unchecked.map(function(t) { return '-' + t; }).join(',') : checked.join(',');",
    ...
    "  window.copyPlaylistHintUrl = function() {",
    "    var code = document.querySelector('.playlist-hint-content code');",
    ...
    "  window.startTagRename = function(span, oldTag) {",
    ...
    "      if(submitted) return;",
    "      submitted = true;",
    "      var newTag = input.value.trim().toLowerCase();",
    "      if(!newTag || (newTag === oldTag)) { restore(); return; }",
    "      fetch('/config/tags/rename', {",
    "        method: 'POST',",
    "        headers: { 'Content-Type': 'application/json' },",
    ...
    "        if(!data.success) {",
    "          restore();",
    "          if(window.showToast) { window.showToast(data.error || 'Failed to rename tag.', 'error'); }",
    "          return;",
    "        }",
    "        applyTagResponse(data, 'Tag renamed: ' + oldTag + ' \\u2192 ' + newTag);",
    "      }).catch(function() {",
    "        restore();",
    "        if(window.showToast) { window.showToast('Failed to rename tag.', 'error'); }",
    "      });",
    "    }",
