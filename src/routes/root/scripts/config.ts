...
export function generateConfigSubtabScript(): string {
  ...
  return [
    ...
    "      const data = { action: 'inline-edit', field: field, key: key, value: newValue };",
    "      try {",
    ...
    "      } else if(d.errors) {",
    "        const errorMsgs = [];",
    "        for(const [ field, message ] of Object.entries(d.errors)) { errorMsgs.push(field + ': ' + message); }",
    ...
    "    try {",
    "      const res = await fetch('/config/service', {",
    ...
    "    try {",
    "      const res = await fetch('/config/channels', {",
    "        body: JSON.stringify({ action: 'inline-edit', field: 'tags', key, value: newValue }),",
    "        headers: { 'Content-Type': 'application/json' },",
    "        method: 'POST'",
    "      });",
    "      const d = await res.json();",
