...
export function generateOverviewContent(baseUrl: string): string {
  ...
  return [
    ...
    "on the desktop, which is what keeps the recording clean. ",
...
export function generateHelpContent(): string {
  ...
  return [
    ...
    "composites, so the window has to be on screen for the recording to be clean. PrismCast minimizes it again as soon as the last capture ends. Channels using ",
    "<strong>native HLS streaming</strong> do not use the window at all, so it stays minimized throughout.</p>",
