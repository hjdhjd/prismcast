...
export function generateHelpContent(): string {
  ...
  return [
    ...
    "composites, so the window has to be on screen for the recording to be clean. PrismCast never brings that window in front of your other applications and ",
    "never changes which tab is selected, apart from the moment a capture starts and the moment a page asks for fullscreen &mdash; and after each, the tab you ",
    "had selected is selected again. If you click a capture tab yourself, that stream records the window's view of that page for about a second, until PrismCast ",
    ...
    "soon as the last capture ends. Channels using <strong>native HLS streaming</strong> do not use the window at all, so it stays minimized throughout.</p>",
