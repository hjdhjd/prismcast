...
export function generateOverviewContent(baseUrl: string): string {
  ...
  return [
    ...
    "on the desktop showing a blank tab, which is what keeps the recording clean. Clicking a capture tab records that window's clipped view of the stream, until ",
    "PrismCast returns to the blank tab moments later. ",
...
export function generateHelpContent(): string {
  ...
  return [
    ...
    "large monitor.</p>",
    "<p>Chrome still needs a display to render into. When that display is smaller than the selected preset, PrismCast notes it once in the log and captures at ",
    "the selected preset anyway &mdash; the window you would see on screen simply cannot show a whole frame.</p>",
    "<p>While a screen-captured channel is streaming, the Chrome window stays un-minimized on the desktop, showing a blank tab. That is expected: capture reads ",
    "what the window composites, so the window has to be on screen for the recording to be clean, and the blank tab in front is what keeps each capture recording ",
    "its full frame. If you click a capture tab, that stream records the window's clipped view of itself until PrismCast returns to the blank tab moments later. ",
    "PrismCast minimizes the window again as soon as the last capture ends. Channels using <strong>native HLS streaming</strong> do not use the window at all, ",
    "so it stays minimized throughout.</p>",
