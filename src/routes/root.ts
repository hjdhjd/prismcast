...
function generateStatusScript(): string {
  ...
  return [
    ...
    "  if (attempts === 0) { return '0 attempts'; }",
    ...
    "    html += '<td class=\"stream-health\">' + getHealthBadge(s.health, s.escalationLevel) + '</td>';",
    "    html += '</tr>';",
    "    if (isExpanded) {",
    ...
    "      html += '<div class=\"details-started\"><strong>Started:</strong> ' + formatTime(s.startTime) + '</div>';",
    "      html += '</div>';",
...
function generateConfigSubtabScript(): string {
  ...
  return [
    ...
    "    var addBtn = document.querySelector('.panel-header .btn-primary');",
...
function generateLandingPageStyles(): string {
  ...
  return [
    ...
    // User channel row tinting to distinguish custom/override channels from built-in.
    ...
    "#import-settings-file, #import-channels-file { display: none; }",
