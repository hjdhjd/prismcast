 ...
 * when probing a device. PrismCast emulates the HDHomeRun CONNECT (an HTTP-streaming model) because the HTTP-streaming family is what every modern HDHR-
 * aware client expects, and because that model has no DRM context to fake. Two call sites consume these strings: the HTTP discovery endpoints (device.xml,
 * discover.json) and the UDP Get-request responder (/sys/* keys). This module is the single source of truth so a future model change is a
...
/**
 * HDHomeRun CONNECT model identifier returned in /discover.json's ModelNumber field. The CONNECT is an HTTP-streaming model - clients that see this
