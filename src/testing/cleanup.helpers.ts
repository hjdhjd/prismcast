 ...
 * after binding (see the source at https://github.com/SamuelScheit/puppeteer-stream/blob/main/src/PuppeteerStream.ts). Upstream issue #154 describes this
 * same WebSocketServer-blocks-exit behavior directly; issue #131 ("Shutdown is taking more than 5 seconds") is a separate report about a setTimeout in the
 * stream-stop path. The pristine fix would be a one-line .unref() upstream; we work around the limitation here so PrismCast tests exit cleanly without
 ...
 *
 * Multiple call patterns are supported because they serve genuinely different test styles:
 *
 ...
 * runner will surface any underlying issue on its own, and a failed close should not fail the test that called this helper. Safe to call more than once:
 * calling close() on an already-closed WebSocketServer is a no-op.
 *
 ...
 *
 * Safe to call more than once in practice: if multiple test files in the same process call this, each schedules an independent unref'd timer; all of them
 * run the close harmlessly because calling closePuppeteerStreamWss again after it has already run does nothing further.
 */
