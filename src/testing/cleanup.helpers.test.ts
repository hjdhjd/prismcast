 ...
 * the awaitable form returns a Promise that always resolves (best-effort), is idempotent, and never throws synchronously; the on-idle form returns void
 * without throwing.
 */
...
describe("closePuppeteerStreamWss", () => {
   ...
   *
   * Instead, the tests pin the contract that matters to consumers: the function returns a Promise that always resolves (best-effort cleanup), is idempotent
   * across repeat calls, and never throws synchronously. The integration is exercised at scale by every test file that calls this helper - if the dynamic
...
describe("closePuppeteerStreamWssOnIdle", () => {
  ...
  test("the scheduled timer is unref'd (does not keep the process alive on its own)", () => {
    ...
    /* We can't directly observe "would this keep the process alive" without forking a subprocess. The unref'd-ness is tested implicitly by the suite - if the
     * timer kept the process alive, the runner would hang at suite end across the dozens of test files that call this helper. The synchronous no-throw check
     * is a proxy: if the unref() call were missing or threw, this assertion would catch it.
     */
    assert.doesNotThrow(() => { closePuppeteerStreamWssOnIdle(); });
  });
});
