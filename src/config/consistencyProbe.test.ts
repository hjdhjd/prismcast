 ...
 * three checks (service tag filter, variant canonical references, domain profile references). The probe runs against the actual loaded module-level state;
...
describe("runConsistencyProbeAtStartup", () => {
  ...
  test("runs to completion without throwing on the default state", async () => {
    ...
    /* The probe iterates registered persistence stores and the loaded user data. Under the unit-test default (no user channels, no user profiles) every check
